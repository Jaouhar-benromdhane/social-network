...
| AQ-AUTH-002 | Are the correct form elements being used in the registration? (Email, Password, First Name, Last Name, Date of Birth, Avatar/Image (Optional), Nickname (Optional), About Me (Optional)) | Oui, tous les champs de register sont presents et un register complet passe en 201 avec ces elements. | frontend/src/App.vue ; M2_REGISTER_CODE=201 ; M2_REGISTER_USER_HAS_FIELDS=1 (script test_m2_validation) | VALIDATED |
...
| ID | Question (copie audit) | Reponse | Preuve | Statut |
|---|---|---|---|---|
| AQ-PROF-001 | Try opening your own profile. Does the profile displays every information requested in the register form, apart from the password? | Oui, /api/profile/me expose email, first/last name, date_of_birth, nickname, about_me, profile_visibility et n expose pas password. | M2_PROFILE_CODE=200 ; M2_PROFILE_HAS_EMAIL=1 ; M2_PROFILE_HAS_FIRST_NAME=1 ; M2_PROFILE_HAS_LAST_NAME=1 ; M2_PROFILE_HAS_DOB=1 ; M2_PROFILE_HAS_NICKNAME=1 ; M2_PROFILE_HAS_ABOUT=1 ; M2_PROFILE_HAS_VISIBILITY=1 ; M2_PROFILE_HAS_PASSWORD_FIELD=0 (script test_m2_validation) | VALIDATED |
...
| AQ-GRP-007 | After being part of a group, can the user create posts and comment already created posts? | Oui, les membres du groupe peuvent creer des posts et commenter les posts existants. | OWNER_POST_CODE=201 ; MEMBER_POST_CODE=201 ; MEMBER_COMMENT_CODE=201 ; OWNER_COMMENT_CODE=201 ; MEMBER_SEES_COMMENTS=1 (script test_group_activity_e2e) | VALIDATED |
| AQ-GRP-008 | Try to create an event in a group. Were you asked for a title, a description, a day/time and at least two options (going, not going)? | Oui, creation event exige title, description, event_datetime et au moins deux options valides. | EVENT_BAD_OPTIONS_CODE=400 ; EVENT_CREATE_CODE=201 ; EVENT_HAS_TITLE=1 ; EVENT_HAS_DESCRIPTION=1 ; EVENT_HAS_DATETIME=1 ; EVENT_HAS_TWO_OPTIONS=1 (script test_group_activity_e2e) | VALIDATED |
| AQ-GRP-009 | Using the same browsers and the same users, after both of them becoming part of the same group, create an event with one of them. Is the other user able to see the event and vote in which option he wants? | Oui, le second membre voit l event et son vote est enregistre correctement. | MEMBER_SEES_EVENT=1 ; MEMBER_VOTE_CODE=200 ; MEMBER_VOTE_PERSISTED=1 ; OPTION1_VOTE_COUNT_ONE=1 (script test_group_activity_e2e) | VALIDATED |

## Chat
| ID | Question (copie audit) | Reponse | Preuve | Statut |
|---|---|---|---|---|
| AQ-CHAT-001 | Try and open two browsers (ex: Chrome and Firefox), log in with different users in each one. Then with one of the users try to send a private message to the other user. Did the other user received the message in realtime? | Oui, message prive envoye et recu en temps reel via websocket. | CHAT_PRIVATE_CODE=201 ; CHAT_PRIVATE_REALTIME=1 (script test_m5_e2e) | VALIDATED |
| AQ-CHAT-002 | Try and open two browsers (ex: Chrome and Firefox), log in with different users that are not following each other at all. Then with one of the users try to send a private message to the other user. Can you confirm that it was not possible to create a chat between these two users? | Oui, creation chat prive refusee si les deux users ne se suivent pas mutuellement. | CHAT_NO_FOLLOW_CODE=403 (script test_m5_e2e) | VALIDATED |
| AQ-CHAT-003 | Using the two browsers with the users start a chat between the two of them. Did the chat between the users went well? (did not crash the server) | Oui, sequence de messages prives stable sans crash serveur. | CHAT_PRIVATE_STABILITY=1 ; CHAT_PRIVATE_HEALTH=200 (script test_m5_e2e) | VALIDATED |
| AQ-CHAT-004 | Try and open three browsers (ex: Chrome and Firefox or a private browser), log in with different users in each one. Then with one of the users try to send a private message to one of the other users. Did only the targeted user received the message? | Oui, seul le destinataire cible recoit le message prive en temps reel. | CHAT_TARGETED_B_GOT=1 ; CHAT_TARGETED_C_GOT=0 (script test_m5_e2e) | VALIDATED |
| AQ-CHAT-005 | Using the three browsers with the users, enter with each user a common group. Then start sending messages to the common chat room using one of the users. Did all the users that are common to the group receive the message in realtime? | Oui, les membres communs du groupe recoivent le message de room en temps reel. | GROUP_CHAT_CODE=201 ; GROUP_CHAT_REALTIME_B=1 ; GROUP_CHAT_REALTIME_C=1 (script test_m5_e2e) | VALIDATED |
| AQ-CHAT-006 | Using the three browsers with the users, continue chatting between the users in the group. Did the chat between the users went well? (did not crash the server) | Oui, chat de groupe stable sur envoi en chaine sans crash serveur. | GROUP_CHAT_STABILITY=1 ; GROUP_CHAT_HEALTH=200 (script test_m5_e2e) | VALIDATED |
| AQ-CHAT-007 | Can you confirm that it is possible to send emojis via chat to other users? | Oui, message emoji transmis et recu correctement en temps reel. | CHAT_EMOJI_CODE=201 ; CHAT_EMOJI_REALTIME=1 (script test_m5_e2e) | VALIDATED |

## Notifications
| ID | Question (copie audit) | Reponse | Preuve | Statut |
|---|---|---|---|---|
| AQ-NOTIF-001 | Can you check the notifications on every page of the project? | Oui, endpoint notifications et affichage frontend fonctionnent pour chaque user connecte. | NOTIF_ENDPOINT_A=200 ; NOTIF_ENDPOINT_B=200 ; NOTIF_ENDPOINT_C=200 ; frontend/src/App.vue (section Notifications) | VALIDATED |
| AQ-NOTIF-002 | Open two browsers, log in as two different private users and with one of them try to follow the other. Did the other user received a notification regarding the following request? | Oui, notif follow_request recue en temps reel et visible dans la liste notifications. | NOTIF_FOLLOW_REQ_CODE=200 ; NOTIF_FOLLOW_WS=1 ; NOTIF_FOLLOW_LIST=1 (script test_m5_e2e) | VALIDATED |
| AQ-NOTIF-003 | Open two browsers, log in with different users on each browser, follow each other and with one of the users create a group and invite the other user. Did the invited user received a notification regarding the group invitation request? | Oui, notif group_invite recue par l user invite. | NOTIF_INVITE_CODE=200 ; NOTIF_INVITE_WS=1 (script test_m5_e2e) | VALIDATED |
| AQ-NOTIF-004 | Open two browsers, log in with different users on each browser, create a group with one of them and with the other send a group entering request. Did the other user received a notification regarding the group entering request? | Oui, notif group_join_request recue par le creator du groupe. | NOTIF_JOIN_REQ_CODE=200 ; NOTIF_JOIN_REQ_WS=1 (script test_m5_e2e) | VALIDATED |
| AQ-NOTIF-005 | Open two browsers, log in with different users on each browser, become part of the same group with both users and with one of the users create an event. Did the other user received a notification regarding the creation of the event? | Oui, notif group_event_created recue par les autres membres du groupe. | NOTIF_EVENT_CREATE_CODE=201 ; NOTIF_EVENT_WS_B=1 ; NOTIF_EVENT_WS_C=1 (script test_m5_e2e) | VALIDATED |
